...
def prepare_reports(dip_data: pd.DataFrame, analysis_day: int, day_col: str) -> io.BytesIO:
    lga, ward = [get_admin_col(dip_data, admin_level, 'raise') for admin_level in ['lga', 'ward']]
    daily_reports = DailyReport(
        dip_data,
        Reporters(cumulative=f'day_{analysis_day}_cumm', day=day_col),
        lga_col=lga,
        ward_col=ward
    )
...
@atimer(title="Gridded Settlement Visitation Analysis")
...
if __name__ == '__main__':
    ...
    folder = "C:\\Workspace\\NIPDs\\MARCH ROUND"
    dip = pd.read_csv(f'{folder}\\obr_march_dip.csv')

    tracks = gpd.read_file(f'{folder}\\GIS\\tracks_day_5.gpkg', driver='GPKG')
    day = 5 #int(input('Enter Analysis Day: '))
    ...
    result = asyncio.run(
        settlement_tracking(
            datasets,
            [State.Kebbi, State.Zamfara, State.Niger],
