...
@timer(display=True)
def generate_settlement_target_area(settlements: gpd.GeoDataFrame, planned_list: gpd.GeoDataFrame=None) -> TADatasets:
    """
    ...
    print(f"Fetching Database Layers for {', '.join(state_info.values())} State(s)....")
    state_boundaries: gpd.GeoDataFrame = ReadDBData(table_ds['state_boundary'], True).read_data({'statename': states})
    building_fps: gpd.GeoDataFrame = ReadDBData(table_ds['buildings_footprint'], True).read_data({'statename': states})
    ...
    for state_name, state_code in tqdm(state_info.items()):
        try:
            ...
            building_fp = building_fps.loc[building_fps['statename']==state_name]
            ...
            print(f"\n\n{e} Error Encountered when Generating Target Area Datasets for {state_name}.\n❌❌Process terminated.")
            break

    return compile_datasets(state_ta_datasets, mlos.global_id, planned_list)
